/**
 * roleAdmin.js —— 由 views/admin-pages/roleAdmin.jade 提取。
 * 懒加载：用户切换到「role-admin」页时由 loader.js 动态拉取。
...
const defaultRoleForm = () => ({
  ...
  const serializeRoleForm = (values) => ({
  ...
  const RoleAdminPage = () => {
    ...
    const [formValues, setFormValues] = useState(defaultRoleForm());
    ...
    const loadRoles = useCallback(async () => {
      ...
      try {
        ...
        const resp = await fetch(`/role-admin/roles?${params.toString()}`);
        if (resp.status === 401) {
        ...
        if (!resp.ok) {
          throw new Error(result.message || '加载角色失败');
        }
    ...
    useEffect(() => {
      if (selectedRow && !roles.some((role) => role.id === selectedRow.id)) {
        setSelectedRow(null);
      }
    }, [roles, selectedRow]);
    ...
    const handleSave = async () => {
      setSaving(true);
      try {
        const payload = serializeRoleForm(formValues);
        if (!payload.realName) {
          throw new Error('请输入角色名称');
        }
        const isEdit = dialogState.mode === 'edit';
        const url = isEdit ? `/role-admin/roles/${formValues.id}` : '/role-admin/roles';
    ...
    const handleDelete = async () => {
      ...
      setSaving(true);
      try {
        const resp = await fetch(`/role-admin/roles/${selectedRow.id}`, { method: 'DELETE' });
        const result = await resp.json();
        if (!resp.ok) {
          throw new Error(result.message || '删除失败');
        }
        setNotify({ open: true, severity: 'success', message: result.message || '角色已删除' });
        setSelectedRow(null);
        await loadRoles();
      } catch (err) {
    ...
    return React.createElement(Box, { sx: { display: 'flex', flexDirection: 'column', gap: 3 } },
      React.createElement(Paper, { sx: { p: 2, borderRadius: 3 } },
        ...
        React.createElement(Stack, { direction: 'row', spacing: 1, flexWrap: 'wrap' },
          React.createElement(Button, { variant: 'contained', size: 'small', onClick: handleOpenCreate }, '新增角色'),
          ...
          React.createElement(Table, { size: 'small' },
            React.createElement(TableHead, null,
              React.createElement(TableRow, null,
                React.createElement(TableCell, null, '角色名称'),
      ...
      React.createElement(Dialog, {
        ...
        React.createElement(DialogTitle, null, dialogState.mode === 'edit' ? '编辑角色' : '新增角色'),
      ...
      React.createElement(Dialog, { open: confirmDelete, onClose: () => setConfirmDelete(false) },
        React.createElement(DialogTitle, null, '删除角色'),
        React.createElement(DialogContent, null,
          React.createElement(Typography, null, '确定删除所选角色吗？')
        ),
        React.createElement(DialogActions, null,
  ...
  window.AdminPages['role-admin'] = RoleAdminPage;
})();
